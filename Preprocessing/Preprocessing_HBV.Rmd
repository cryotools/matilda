...
home <- "/home/ana/Seafile/Ana-Lena_Phillip/data/"
```
# Meteorological Data from ERA5
```{r}
# Temperature and Precipitation, Units: Temp in K and Prec in mm
era5_met <- read.csv(paste0(home, "input_output/input/No1_Urumqi_ERA5_2000_201907.csv"), dec=".", sep = ",", header=TRUE)
era5_met <- era5_met[-c(1,2)]

era5_met$Date <- ymd_hms(era5_met$Date)
era5_met <- era5_met[era5_met$Date >= "2011-01-01" & era5_met$Date <= "2018-12-31",]

# Unit Conversion
era5_met$t2m <- era5_met$t2m - 273.15 # now °C

# Daily values
era5_met_daily <- mutate(era5_met, Date = floor_date(Date, "day")) %>%
group_by(Date) %>%
summarize(tp = sum(tp), t2m = mean(t2m))
...
water_density = 1000
era5_met_daily$PE_calc <- ifelse((era5_met_daily$t2m + 5 > 0), (((solar_constant/(water_density*latent_heat_flux))*((era5_met_daily$t2m+5)/100))), 0)
...
ptq_obs <- subset(era5_daily, select=c("Date", "tp", "t2m"))
colnames(ptq_obs) <- c("Date", "P", "T")
...
ptq_obs$Q <- as.numeric(ptq_obs$Q)
...
evap_calc <- mutate(era5_daily, month=month(Date), day=day(Date)) %>% 
  group_by(month, day) %>% 
...
# Output for HBV
```{r}
setwd("~/Seafile/Ana-Lena_Phillip/data/scripts/HBV_Model/HBV-light_data/Glacier_No.1/")
write.table(ptq_obs, "ERA5_evap_calc/Data/ptq.txt", sep="\t", row.names=FALSE)
write.table(evap_calc, "ERA5_evap_calc/Data/evap.txt", sep="\t", row.names=FALSE)

write.table(ptq_obs, "ERA5_evap/Data/ptq.txt", sep="\t", row.names=FALSE)
write.table(evap_era, "ERA5_evap/Data/evap.txt", sep="\t", row.names=FALSE)

write.table(ptq_era, "ERA5_runoff/Data/ptq.txt", sep="\t", row.names=FALSE)
write.table(evap_era, "ERA5_runoff/Data/evap.txt", sep="\t", row.names=FALSE)
```
# Comparison
```{r}
