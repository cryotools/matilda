...
cmip_df_temp = cmip_df_temp.groupby(["month", "year"], as_index=False).mean()
cmip_df_prec = cmip_df_prec.groupby(["month", "year"], as_index=False).sum()
...
for i in range((period_start+period_length), period_end, period_length):
    monthly_trend_cmip["diff_hist_"+str(i+(period_length-1))] = monthly_trend_cmip["period_" + str(i) + "_" + str(i + (period_length - 1))] - monthly_trend_cmip["period_" + str(i-period_length)+ "_" + str(i-1)]
