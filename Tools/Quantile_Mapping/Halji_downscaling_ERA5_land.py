...
# era5_land_file = working_directory + '/ERA5_land/20200302_Halji_ERA5_land_1981_201911_compressed.nc'
# temperatures_for_qmap = 'QMAP/20200302_temperatures_for_qmap.csv'
# era5L_temp_for_qmap = 'QMAP/20200302_Halji_ERA5_land_T2_2000_2019_closest_gp.csv'
# mapped_temperatures = 'QMAP/20200302_mapped_T2.csv'
# output_csv = working_directory + 'input/20200703_Halji_ERA5_land_1981_202004_closest_gp.csv'
# plt_dir = working_directory + 'plots/comparison_AWS_datasets/0703/' + (output_csv.split('/input/')[1]).split('.csv')[0] + '_air_presurre_diff_mean_measured/'; print(plt_dir)

era5_land_file = working_directory + '/ERA5_land/20200701_Halji_ERA5L_1981_202004_compressed.nc'
temperatures_for_qmap = 'QMAP/20200701_temperatures_for_qmap.csv'
era5L_temp_for_qmap = 'QMAP/20200701_Halji_ERA5_land_T2_2000_2019_closest_gp.csv'
mapped_temperatures = 'QMAP/20200701_mapped_T2.csv'
output_csv = working_directory + 'input/20200830_Halji_ERA5L_1981_202004_closest_gp.csv'
plt_dir = working_directory + 'plots/comparison_AWS_datasets/0703/' + (output_csv.split('/input/')[1]).split('.csv')[0] + 'newest_precipitation/'; print(plt_dir)

target_altitude = 5359
timezone_difference_to_UTC = 6
margin = 0.2
z0 = 0.00212                                # (m) mean between roughness firn 4 mm and fresh snow 0.24 mm
find_point_and_calculate_data = False
store_netCDF = True
plots = True
...
era5_land.t2m.to_pandas().to_csv(working_directory + era5L_temp_for_qmap)
...
TEMP_es = calculate_ew(era5_land_compare.t2m.values)
TEMP_saturation_mixing_ratio_vapour = 0.622 * (TEMP_es / (era5_land_compare['sp'].values * 100))
TEMP_simulated_sh = (simulated_rh2 / 100) * TEMP_saturation_mixing_ratio_vapour
...
if find_point_and_calculate_data is True:
    ...
    df_for_R.to_csv(working_directory + temperatures_for_qmap, index=False)
    temperature = pd.read_csv(working_directory + mapped_temperatures).values[:,0]
    ...
    ### Barometric equation! Steigung hier besser! Auch besser als diff mean best version!
    air_pressure = ((era5_land['sp'].values)/100) * (1-abs(lapse_rate_temperature.values)*height_diff/era5_land['t2m'].values) ** ((g*M)/(R*abs(lapse_rate_temperature.values)))

    ### over SLP
    #PRES = df[PRES_var].values * np.power((1 - (0.0065 * hgt)/(288.15)), 5.22)  # Pressure
    #SLP = (era5_land['sp'].values/100) / np.power((1 - (0.0065 * 5154.) / (288.15)), 5.255)
    #air_pressure = SLP * np.power((1 - (0.0065 * target_altitude) / (288.15)), 5.255)
    ...
    total_precipitation_scaling = 2.023031162 ### old scaling: total_precipitation_scaling = 2.1010939
    ...
    total_precipitation = total_precipitation * 1000 * total_precipitation_scaling
    ...
    #temperature = (era5_land['t2m'].values)
    #air_pressure = ((era5_land['sp'].values) / 100)
    #U2 = U10 * (np.log(2 / z0) / np.log(10 / z0))
...
ds_era = xr.Dataset()
ds_era.coords['time'] = era5_land.time[1:] #+ pd.Timedelta(hours=timezone_difference_to_UTC)
...
if plots == True:
    from misc_functions.plot_functions import plot_line_diff, scatterplot_linear, plot_cdf, plot_cdf_compare_3var, plot_cdf_compare_4var, plot_pdf_3var, plot_line
    dataset_name = 'ERA5_land'
    #import os; os.mkdir(plt_dir)

    start_date_temp = '20100101'; end_date_temp = ds_era.time.values[-1]
    ds_era_line_plot = ds_era.sel(time=slice(start_date_temp, end_date_temp))
    def plot_line_era5(field,temperature='K'):
        plot_line(ds_era_line_plot[field], ds_era_line_plot[field].long_name, '', ds_era_line_plot[field].units, plt_dir, 'h',name='ERA5_timeline', temperature=temperature)

    #plot_line_era5('T2', temperature = 'C')
    #plot_line_era5('LWin')
    #plot_line_era5('RH2')
    #plot_line_era5('PRES')
    #plot_line_era5('G')
    ...
    def plot_compare_scatter_era_aws(field, line_best_fit = False):
        scatterplot_linear(ds_era[field].resample(time=integration).mean(), aws[field].resample(time=integration).mean(),
                        'ERA5-Land', 'AWS', ds_era[field].long_name, integration, plt_dir, str(integration), ds_era[field].units, ds_era[field].units, line_best_fit = line_best_fit)
    ...
    def plot_cdf_curves(measured, simulated, field, temperature='K'):
        plot_cdf_compare_3var(measured, simulated, ds_era[field][~np.isnan(aws[field])], ds_era[field].units, ds_era[field].long_name, 'AWS', 'ERA5 raw', 'ERA5 downscalled', plt_dir, name=False, save=True)
    ...
    plot_cdf_curves(measured_temperatures, simulated_temperatures, 'T2')
    ### Problem solve
    plot_cdf_compare_3var(aws.SH, ds_era['SH'][~np.isnan(aws['SH'])], TEMP_simulated_sh * 100000, ds_era['SH'].units, ds_era['SH'].long_name, 'AWS', 'ERA5 raw', 'ERA5 downscalled', plt_dir, name=False, save=True)
    #plot_cdf_curves(aws.SH,  * 100000, 'SH')
    ...
    def plot_compare_era_aws(field,temperature='K'):
        plot_line_diff(ds_era[field].resample(time=integration).mean(),aws[field].resample(time=integration).mean(),
                       'ERA5', 'AWS', ds_era[field].long_name, str(integration), ds_era[field].units, plt_dir, integration,temperature=temperature)
    ...
    scatterplot_linear(ds_era[field].resample(time=integration).mean(), aws_tp[field].resample(time=integration).mean(), dataset_name, 'AWS', ds_era[field].long_name, integration, plt_dir, str(integration), ds_era[field].units, ds_era[field].units, line_best_fit = True)
